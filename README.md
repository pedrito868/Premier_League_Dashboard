...
* [cite_start]**Player Selection**: Choose any player from the 2022-2023 season (who played a minimum of 900 minutes).
* [cite_start]**Performance Percentile Plot**: A polar (radar) chart visualizes the player's percentile ranks against all other players in seven key metrics:
    * Goals per 90
    * Assists per 90
...
* [cite_start]**Key Stat Boxes**: At-a-glance view of the selected player's core stats, including total goals, assists, minutes played, team, nationality, and year of birth.
...
* **Player Comparison (Scatter Plot)**: An interactive `plotly` scatter plot that lets you compare any two statistics from the dataset (e.g., `xG_p90` vs. `xAG_p90`).
    * [cite_start]Points are color-coded by position (GK, DF, MF, FW).
    * [cite_start]Point size is relative to the player's total minutes played.
    * [cite_start]Hover over any point to see the player's name.
* [cite_start]**Position Comparison (Box Plot)**: An interactive `plotly` box plot to visualize the distribution of any selected statistic across the four main player positions, helping to identify positional trends.
...
[cite_start]The data used in this project (`Players.xlsx`) was sourced from **fbref.com** for the 2022-2023 Premier League season.

[cite_start]The raw data was processed within the R script to:
